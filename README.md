...
手动管理热词 + 自动学习修正。热词同时传入 ASR 和 LLM，提升专有名词识别准确率。
...
编辑历史记录时自动捕获 diff，LLM 验证修正合理性，智能生成热词。
...
持久化存储识别历史，支持关键词搜索、编辑、复制、重新插入、CSV 导出。
...
主窗口内直接配置热键、ASR 凭证、LLM 设置、热词管理，支持 API 连接测试。
...
├── LLM/                           # DeepSeek LLM 后处理 + Prompt 构建
├── Input/                         # 全局快捷键管理
├── Services/                      # 凭证存储、历史记录、热词管理、词级 Diff
└── UI/
